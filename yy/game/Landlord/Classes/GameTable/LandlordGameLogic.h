#ifndef __Landlord_GameLogic_h__
#define __Landlord_GameLogic_h__
...
#define KING_COUNT								2				//所有王的个数
	...
	public:
		...
		//获取扑克花色(默认为真实花色)
		BYTE GetCardHuaKind(BYTE iCard, INT bTrueHua=true);
		//获取扑克相对大小(默认为牌大小,非排序大小)
		int GetCardBulk(BYTE iCard, bool bExtVal=false);
		//获取扑克牌通过相对大小
		BYTE GetCardByValue(int iCardValue);
		//設置王可以當牌
		void SetKingCanReplace(INT bKingCanReplace=false){m_bKingCanReplace=bKingCanReplace;}
		//獵取王是否可以當牌
		INT GetKingCanReplace(){return m_bKingCanReplace;}
		//设置排序方式
	...
	public:
		//排列扑克,按大小(保留系统序例)
		INT SortCard(BYTE iCardList[], BYTE bUp[], BYTE iCardCount,INT bSysSort = false);
		//反转牌顺(从低->高)
		INT ReverseCard(BYTE iCardList[], BYTE bUp[], BYTE iCardCount);
		//按牌型排序
		INT SortCardByStyle(BYTE iCardList[],BYTE iCardCount);
		//按花色排序
		INT SortCardByKind(BYTE iCardList[],BYTE iCardCount);
	public:
		//混乱扑克
	...
	public://[辅助函数]
		...
		//是否为同一数字牌
		INT IsSameNumCard(BYTE iCardList[],int iCardCount,bool bExtVal=false);
		//是否为同一花色
		INT IsSameHuaKind(BYTE iCardList[],int iCardCount,bool bExtVal=false);
		//查找 >=4 炸弹的数量炸弹基数
		...
		//是否为某指定的顺子(变种顺子)
		INT IsVariationSequence(BYTE iCardList[], int iCardCount, int iCount);
		//是否为某指定的顺子
		INT IsSequence(BYTE iCardList[], int iCardCount, int iCount);
		//提取指定的牌
		BYTE TackOutBySpecifyCard(BYTE iCardList[], int iCardCount,BYTE bCardBuffer[],int &iResultCardCount,BYTE bCard);
		//提取某张指定数字的牌
		INT TackOutCardBySpecifyCardNum(BYTE iCardList[],int iCardCount,BYTE iBuffer[],int &iBufferCardCount,BYTE iCard,INT bExtVal=false);
	...
	public:
		...
		//是否单牌
		inline INT IsOnlyOne(BYTE iCardList[], int iCardCount) { return iCardCount==1; };
		//是否对牌
		INT IsDouble(BYTE iCardList[], int iCardCount,bool bExtVal=false);
		//3 带 1or2(带一对带二单张或带一单张
		INT IsThreeX(BYTE iCardList[], int iCardCount, int iX/*1or2*/, bool bExtVal=false);
		//王炸
		INT IsKingBomb(BYTE iCardList[],int iCardCount);
		//4+张牌 炸弹
		INT IsBomb(BYTE iCardList[], int iCardCount, bool bExtVal=false);
		//同花炸弹
		INT IsBombSameHua(BYTE iCardList[],int iCardCount);
		//同花(非顺子)
		INT IsFlush(BYTE iCardList[],int iCardCount);
		//是否是同花顺
		INT IsStraightFlush(BYTE iCardList[], int iCardCount, bool bExtVal=false);
		//变种单甩
		INT IsVariationStraight(BYTE iCardList[],int iCardCount,bool bExtVal=false);
		//单甩
		INT IsStraight(BYTE iCardList[],int iCardCount,bool bExtVal=false);
		//变种对甩
		INT IsVariationDoubleSequence(BYTE iCardList[],int iCardCount,bool bExtVal=false);
		//对甩 //连对?
		INT IsDoubleSequence(BYTE iCardList[],int iCardCount,bool bExtVal=false);

		//是否变种是连续的三带X(0,1,2,3)
		INT IsVariationThreeXSequence(BYTE iCardList[], int iCardCount, int iSeqX/*0,1or2*/, bool bExtVal=false);

		//是否是连续的三带X(0,1,2,3)
		INT IsThreeXSequence(BYTE iCardList[], int iCardCount, int iSeqX/*0,1or2*/, bool bExtVal=false);
		//是否三顺带二顺(蝴蝶)
		INT IsThreeSequenceDoubleSequence(BYTE iCardList[], int iCardCount, bool bExtVal=false);
		//是否三顺带二顺(蝴蝶)
		INT IsVariationThreeSequenceDoubleSequence(BYTE iCardList[], int iCardCount, bool bExtVal=false);
		//510K 炸弹
		INT IsSlave510K(BYTE iCardList[], int iCardCount, bool bExtVal=false);
		//510K 同花炸弹
		INT IsMaster510K(BYTE iCardList[], int iCardCount, bool bExtVal=false);
		//四带一或者四带二
		INT IsFourX(BYTE iCardList[],int iCardCount,int iX/*1or 2*/);//单张1,任意张2,一对子,2对4
		//是否变种四带X顺
		INT IsVariationFourXSequence(BYTE iCardList[],int iCardCount,int iSeqX);
		//四带一或者四带二的顺子
		INT IsFourXSequence(BYTE iCardList[],int iCardCount,int iSeqX);
		//[出牌相关]
	public:
		//自动出牌函数
		INT AutoOutCard(BYTE iHandCard[], int iHandCardCount, BYTE iBaseCard[], int iBaseCardCount, BYTE iResultCard[], int & iResultCardCount, INT bFirstOut);
		//是否可以出牌
		...
		//查找比当前出牌大的
		INT TackOutCardMoreThanLast(BYTE iHandCard[], int iHandCardCount,BYTE iBaseCard[], int iBaseCardCount,
	...
	public:	
		...
		//提取2个以上连续的三带1,2
		INT TrackOut3XSequence(BYTE iCardList[], int iCardCount, BYTE iBaseCard[], int iBaseCount, BYTE iResultCard[], int &iResultCardCount, int xValue);
		//提取2个以上连续的三带1,2
		INT TrackOut3Sequence2Sequence(BYTE iCardList[], int iCardCount, BYTE iBaseCard[], int iBaseCount, BYTE iResultCard[], int &iResultCardCount);
		//提取2个以上连续的三带1,2
		...
		//提取单张的顺子,连对顺子,连三顺子
		INT TackOutSequence(BYTE iCardList[],int iCardCount,BYTE iBaseCard[],int iBaseCount,BYTE iResultCard[],int &iResultCardCount,int xSequence,INT bNoComp = false);
		//提取同花顺
		INT TackOutStraightFlush(BYTE iCardList[],int iCardCount,BYTE iBaseCard[],int iBaseCount,BYTE iResultCard[], int &iResultCardCount);
		...
		//提取所的炸弹
		INT TackOutAllBomb(BYTE iCardList[],int iCardCount,
			BYTE iResultCard[],int &iResultCardCount,int iNumCount=4);
		//提取炸弹(张数默认为4)
		INT TackOutBomb(BYTE iCardList[],int iCardCount,BYTE iResultCard[],int &iResultCardCount,int iNumCount=4);
		//提取王炸
		INT TackOutKingBomb(BYTE iCardList[],int iCardCount,BYTE iResultCard[],int &iResultCardCount);
		//提取510K
		INT TrackOut510K(BYTE iCardList[],int iCardCount,BYTE iResultCard[],int &iResultCardCount, bool bExtVal=false);
		//测试510K
		INT Test510K(BYTE iCardList[],int iCardCount,bool bExtVal=false);
		//拷背
		INT Copy510K(BYTE iCardList[],int iCardCount,BYTE iResultCard[],int &iResultCardCount);
		//拆大桌面牌
		INT TackOutCardByNoSameShape(BYTE iCardList[],int iCardCount,BYTE iResultCard[],int &iResultCardCount,BYTE iBaseCard[],int iBaseCardCount );
		///提取指定的牌
		INT TackOutCardBySpecifyCard(BYTE iCardList[],int iCardCount,BYTE iResultCard[],int &iResultCardCount,BYTE iBaseCard[],int iBaseCardCount ,BYTE iSepcifyCard);
		//用大的牌牌大桌面上的牌
		INT TackOutMoreThanLastShape(BYTE iCardList[],int iCardCount,BYTE iResultCard[],int &iResultCardCount,BYTE iBaseCard[],int iBaseCardCount);
		//分析函数
