...
//获取扑克花色
BYTE GameLogic::GetCardHuaKind(BYTE iCard, INT bTrueHua)
{ 
...
int GameLogic::GetCardBulk(BYTE iCard, bool bExtVal)
{
...
//从值得到牌
BYTE GameLogic::GetCardByValue(int Value)
{
	BYTE CardArray[55]={
		0x00,
		0x01,0x11, 0x21,0x31,
...
//按牌面数字从大到小排列扑克
INT GameLogic::SortCard(BYTE iCardList[], BYTE bUp[], BYTE iCardCount,INT bSysSort)
{
...
INT GameLogic::ReverseCard(BYTE iCardList[], BYTE bUp[], BYTE iCardCount)
{
	BYTE iTemp;
...
//按牌型排序
INT GameLogic::SortCardByStyle(BYTE iCardList[], BYTE iCardCount)
{
	//如果排序设置是要求按大小排序
...
//按花色排序
INT GameLogic::SortCardByKind(BYTE iCardList[],BYTE iCardCount)
{
	return true;
}

//混乱扑克,服务器使用
BYTE GameLogic::RandCard(BYTE iCard[], int iCardCount,int iRoomId,bool bHaveKing)
{
...
//删除扑克
int GameLogic::RemoveCard(BYTE iRemoveCard[],   //要删除的牌面
...
//清除 0 位扑克
int GameLogic::RemoveNummCard(BYTE iCardList[], int iCardCount)
{
	int iRemoveCount=0;
	for (int i=0;i<iCardCount;i++)
	{
...
//比较单张
INT GameLogic::CompareOnlyOne(BYTE iFirstCard, BYTE iNextCard)
{
...
//计算牌里的分数(5,10,K)
int GameLogic::FindPoint(BYTE iCardList[], int iCardCount)
{
	int iPoint = 0; //分数
	for (int i=0; i<iCardCount; i++)
	{
...
//几张牌是否是相同数字
INT GameLogic::IsSameNumCard(BYTE iCardList[],int iCardCount,bool bExtVal)
{
	int i, temp[18] = {0};
...
//是否为同花
INT GameLogic::IsSameHuaKind(BYTE iCardList[], int iCardCount, bool bExtVal)
{
...
//查找用户手中炸弹数
BYTE GameLogic::GetBombCount(BYTE iCardList[],int iCardCount,int iNumCount,bool bExtVal)
{
	int iCount = 0,
		temp[18] = {0};
	for(int i=0; i<iCardCount; i++)
...
//获取指定张数牌个数
BYTE GameLogic::GetCountBySpecifyNumCount(BYTE iCardList[],int iCardCount,int Num)
{
	BYTE temp[18] = {0};
	int count = 0;
	for(int i = 0;i < iCardCount;i ++)
...
//获取指定牌个数
BYTE GameLogic::GetCountBySpecifyCard(BYTE iCardList[],int iCardCount,BYTE bCard)
{
	int count = 0;
	for(int i = 0;i < iCardCount;i ++)
...
//获取指定牌张数牌大小
BYTE GameLogic::GetBulkBySpecifyCardCount(BYTE iCardList[],int iCardCount,int iCount)
{
	BYTE temp[18] = {0};
	for(int i = 0;i < iCardCount;i ++)
		temp[GetCardBulk(iCardList[i])]++;
...
//是否为变种顺子
INT GameLogic::IsVariationSequence(BYTE iCardList[], int iCardCount, int iCount)
{
...
//是否為順子
INT GameLogic::IsSequence(BYTE iCardList[], int iCardCount, int iCount)
{
	BYTE temp[18]={0};
	for(int i= 0;i < iCardCount;i++)
	{
...
//提取指定牌返回找到牌個數
BYTE  GameLogic::TackOutBySpecifyCard(BYTE iCardList[], int iCardCount,BYTE bCardBuffer[],int &iResultCardCount,BYTE bCard)
{
	iResultCardCount = 0;
	for(int i = 0;i < iCardCount;i++)
	{
...
//提取1,2,3 or 4张相同数字的牌
int GameLogic::TackOutBySepcifyCardNumCount(BYTE iCardList[], int iCardCount, 
...
//提取指定花色牌
int GameLogic::TackOutByCardKind(BYTE iCardList[],int iCardCount,BYTE iDoubleBuffer[],BYTE iCardKind)
{
	int count =0;
...
//拆出(将手中牌多的拆成少的)
int GameLogic::TackOutMuchToFew(BYTE iCardList[],int iCardCount,BYTE iDoubleBuffer[],int &iBufferCardCount,BYTE iCardMuch,BYTE iCardFew)
{
	iBufferCardCount=0;
	int count =0;
	BYTE iBuffer[54];
...
//提取某张指定大小的牌
INT GameLogic::TackOutCardBySpecifyCardNum(BYTE iCardList[],int iCardCount,BYTE iBuffer[],int &iBufferCardCount,BYTE iCard,INT bExtVal)
{
	iBufferCardCount = 0;
...
BYTE  GameLogic::GetSerialByMoreThanSpecifyCard(BYTE iCardList[], int iCardCount,
...
int  GameLogic::GetSerialBySpecifyCard(BYTE iCardList[],int iStart,int iCardCount,BYTE iCard)
{
...
//变种顺子中最大的
BYTE GameLogic::GetBulkBySpecifyVariationSequence(BYTE iCardList[],int iCardCount ,int iSequence )
{
...
BYTE  GameLogic::GetBulkBySpecifySequence(BYTE iCardList[],int iCardCount ,int iSequence )
{
	int temp[18] = {0};
	for(int i = 0; i < iCardCount; i++)
	{
...
//找出一个最小或最大的牌
int  GameLogic::GetBulkBySepcifyMinOrMax(BYTE iCardList[], int iCardCount, int MinOrMax/*1 or 255*/, bool bExtVal)
{
...
* @param bExlVol ?
*/
BYTE GameLogic::GetCardShape(BYTE iCardList[], int iCardCount, bool bExlVol)
{
...
//对牌
INT GameLogic::IsDouble(BYTE iCardList[], int iCardCount, bool bExtVal)
{
	if (iCardCount != 2) 
		return false;
...
//3 带 0,1or2,or3
INT GameLogic::IsThreeX(BYTE iCardList[], int iCardCount, int iX, bool bExtVal)
{
...
//四带1or2
INT GameLogic::IsFourX(BYTE iCardList[],int iCardCount,int iX)
{
...
//王炸
INT GameLogic:: IsKingBomb(BYTE iCardList[],int iCardCount)			//是否为王炸(抓到所的王)
{
...
//4+张牌 炸弹
INT GameLogic::IsBomb(BYTE iCardList[], int iCardCount, bool bExtVal)
{
	if(iCardCount < 4)
		return false;
...
//同花炸弹
INT GameLogic::IsBombSameHua(BYTE iCardList[],int iCardCount)
{
...
//同花(非同花)
INT GameLogic::IsFlush(BYTE iCardList[],int iCardCount)
{
	return IsSameHuaKind(iCardList, iCardCount);
}

//同花顺 5张同花连续牌
INT GameLogic::IsStraightFlush(BYTE iCardList[], int iCardCount, bool bExtVal)
{
...
INT GameLogic::IsVariationStraight(BYTE iCardList[],int iCardCount,bool bExtVal)
{
	if(iCardCount < 5)
		return false;
...
//是否是顺子指定张数
INT GameLogic::IsStraight(BYTE iCardList[],int iCardCount,bool bExtVal)
{
	if(iCardCount < 5)
		return false;
...
//是否是变种连对AA22或2233等
INT GameLogic::IsVariationDoubleSequence(BYTE iCardList[],int iCardCount,bool bExtVal)
{
...
//是否是连对
INT GameLogic::IsDoubleSequence(BYTE iCardList[],int iCardCount,bool bExtVal)
{
...
//变种三顺带二顺
INT GameLogic::IsVariationThreeSequenceDoubleSequence(BYTE iCardList[], int iCardCount, bool bExtVal)
{
	if(iCardCount < 10)		//三顺至少2二顺也至少二
		return false;

	BYTE iBuffer3[54],iBuffer2[54];
	INT bValue3 = false,bValue2 = false;	//三顺,二顺是否为顺,
...
//三顺带二顺
INT GameLogic::IsThreeSequenceDoubleSequence(BYTE iCardList[], int iCardCount, bool bExtVal)
{
	if(iCardCount < 10)		//三顺至少2二顺也至少二
		return false;
...
//变种连三带x
INT GameLogic::IsVariationThreeXSequence(BYTE iCardList[], int iCardCount, int iSeqX, bool bExtVal)
{
...
//连的三带 0,1 or 2
INT GameLogic::IsThreeXSequence(BYTE iCardList[], int iCardCount, int iSeqX, bool bExtVal)
{
	if(iCardCount < 6)		//三顺至少2
		return false;
...
//变种四顺带　
INT GameLogic::IsVariationFourXSequence(BYTE iCardList[],int iCardCount,int iSeqX)
{
...
//四顺带　
INT GameLogic::IsFourXSequence(BYTE iCardList[],int iCardCount,int iSeqX)
{
	if(iCardCount < 8)		//四顺至少2
		return false;
...
//判断是否是510K 炸弹
INT GameLogic::IsSlave510K(BYTE iCardList[], int iCardCount, bool bExtVal)
{
...
//判断是否是510K 同花
INT GameLogic::IsMaster510K(BYTE iCardList[], int iCardCount, bool bExtVal)
{
...
//自动找出可以出的牌
INT GameLogic::AutoOutCard(BYTE iHandCard[], int iHandCardCount, //当前玩家手中所有的牌数据
									BYTE iBaseCard[], int iBaseCardCount, //前一个出牌的人出的牌数据
									BYTE iResultCard[], int & iResultCardCount, //找到的结果
									INT bFirstOut) //当前玩家是否先手
{
	iResultCardCount = 0;
...
//查找一个比当前大的
INT GameLogic::TackOutCardMoreThanLast(BYTE iHandCard[], int iHandCardCount,
...
//直接提取比桌面上大的牌型
INT  GameLogic::TackOutMoreThanLastShape(BYTE iCardList[], int iCardCount, 
												  BYTE iResultCard[], int &iResultCardCount, 
												  BYTE iBaseCard[], int iBaseCardCount)
{
	iResultCardCount = 0;
...
BYTE GameLogic::TackOutThreeX(BYTE iCardList[], int iCardCount, 
...
//提取蝴蝶
INT  GameLogic::TrackOut3Sequence2Sequence(BYTE iCardList[], int iCardCount, BYTE iBaseCard[], int iBaseCount, BYTE iResultCard[], int &iResultCardCount)
{
	iResultCardCount = 0;
...
//提取指定三条带顺
INT  GameLogic::TrackOut3XSequence(BYTE iCardList[], int iCardCount, BYTE iBaseCard[], int iBaseCount, BYTE iResultCard[], int &iResultCardCount, int xValue)
{
	iResultCardCount=0;
	if(iCardCount < iBaseCount)	//张数不够
		return false;
...
//重写提取单张的顺子,连对 or 连三
INT GameLogic::TackOutSequence(BYTE iCardList[], int iCardCount, //手中的牌
										BYTE iBaseCard[], int iBaseCount,   //桌面上最大的牌, 牌的个数
										BYTE iResultCard[], int &iResultCount, //找到的牌
										int xSequence,INT bNoComp)							//顺子的个数
{
	iResultCount=0;
	BYTE iTack[54];
...
//提取同花順
INT GameLogic::TackOutStraightFlush(BYTE iCardList[],int iCardCount,BYTE iBaseCard[],int iBaseCount,BYTE iResultCard[], int &iResultCardCount)
{
	iResultCardCount=0;
	if(iCardCount < iBaseCount)
		return false;
...
//得到顺子的起始位置
int GameLogic::GetSequenceStartPostion(BYTE iCardList[],int iCardCount,int xSequence)
{
	BYTE temp[18]={0};
	int Postion=0;
	for(int i=0;i<iCardCount;i++)
...
/*
//提取单张的顺子,连对 or 连三
BYTE GameLogic::TackOutSequence(BYTE iCardList[], int iCardCount, //手中的牌
BYTE iBaseCard[], int iBaseCount,   //桌面上最大的牌, 牌的个数
BYTE iResultCard[], int &iResultCount //找到的牌
)
{
...
/*BYTE GameLogic::TackOutStraightFlush(BYTE iCardList[], int iCardCount, 
BYTE iBaseCard[], int iBaseCount, 
BYTE iResultCard[], int &iResultCardCount, 
int bExtVal)
{
...
//提取510K
INT GameLogic::TrackOut510K(BYTE iCardList[],int iCardCount,BYTE iResultCard[],int &iResultCardCount, bool bExtVal)
{
	iResultCardCount=0;
	BYTE temp[48]={0};
	BYTE huasei[4][16]={0};
...
//复制510K
INT GameLogic::Copy510K(BYTE iCardList[],int iCardCount,BYTE iResultCard[],int &iResultCardCount)
{
	iResultCardCount=0;
	BYTE five,ten,k;
	for(int i=0;i<iCardCount;i++)
...
//测试是否为5,10k
INT GameLogic::Test510K(BYTE iCardList[],int iCardCount, bool bExtVal)
{
	INT five=false,ten=false,k=false;
	for(int i=0;i<iCardCount;i++)
	{
		if(GetCardNum(iCardList[i])==5)
			five=true;
...
//拆大
INT GameLogic::TackOutCardByNoSameShape(BYTE iCardList[],int iCardCount,BYTE iResultCard[],int &iResultCardCount,BYTE iBaseCard[],int iBaseCardCount )
{
	if(iCardCount <1)
	{
		return false ; 
...
///根据指定的牌提取
INT GameLogic::TackOutCardBySpecifyCard(BYTE iCardList[],int iCardCount,BYTE iResultCard[],int &iResultCardCount,BYTE iBaseCard[],int iBaseCardCount ,BYTE iSepcifyCard)
{
	if(iCardCount <1)
	{
		return false ; 
...
//是否可以出牌
bool GameLogic::CanOutCard(BYTE iOutCard[], int iOutCount,  //要出的牌
...
/*
//查找三张的个数
int GameLogic::SearchCountOfThree(BYTE iCardList[],int iCardCount,bool bExtVal)
{
int temp[17]={0};
for(int i = 0; i < iCardCount; i++)
{
...
/*
//查找三张的点数
int GameLogic::SearchThreeCard(BYTE iCardList[],int iCardCount,bool bExtVal)
{
int temp[17]={0};
...
/*
//查找三连牌中最小的那个三张的牌点
int GameLogic::SearchMinThreeSeq(BYTE iCardList[], int iCardCount, bool bExtVal)
{
int temp[17] = {0};
for(int i = 0; i < iCardCount; i++)
{
...
INT GameLogic::TackOutAllBomb(BYTE iCardList[],int iCardCount,
...
//提取炸弹
INT GameLogic::TackOutBomb(BYTE iCardList[], int iCardCount,
									BYTE iResultCard[], int &iResultCardCount,int iNumCount)
{
	iResultCardCount=0;
	BYTE bCardBuffer[54];
...
//提取王炸
INT GameLogic::TackOutKingBomb(BYTE iCardList[],int iCardCount,BYTE iResultCard[],int &iResultCardCount)
{
	iResultCardCount=0;
...
///拖动选择牌
void GameLogic::DragCardAutoSetValidCard(BYTE iUpCardList[], int iUpCardCount, BYTE bResult[], int & bResultCount)
{
...
///玩家点击牌之后智能提取牌控件
void  GameLogic::AITrackOutCard(BYTE  iCardList[] ,int iCardCount,     //玩家的手牌数据
										...
										BYTE  bResult[], int & bResultCount )      ///提取到的结果
{
	...
	if(0 == iBaseCardCount)
	{
		if(TackOutCardBySpecifyCard(iCardList ,iCardCount ,bResult ,bResultCount ,iTempBaseCardList , iTempBaseCardCount, iCardValue))
		{
			return ;
		}
	}
...
//找出一个最小或最大的牌
BYTE GameLogic::GetCardMinOrMax(BYTE iCardList[], int iCardCount, int MinOrMax/*1 or 255*/, bool bExtVal)
{
	int nIndex = 0;
	int CardNum;
