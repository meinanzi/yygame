...
namespace XZMJ
{
	...
	bool SouthMahjongCardPool::init(INT count)
	{
		...
		_cardSize = Sprite::createWithSpriteFrameName("DaoDaTiao1.png")->getContentSize();
		auto sp = Sprite::createWithSpriteFrameName("DaoDaTiao1.png");
		auto origin = GameManager::getInstance()->getNewOrigin();
		_startSortPos = origin + COCOS_NODE(Layout, "handPanel1")->getPosition() + Vec2(sp->getContentSize().width/2.0f, COCOS_NODE(Layout, "handPanel1")->getContentSize().height/2);
	...
	void SouthMahjongCardPool::setHandCardPos(Card* noCard)
	{
		...
		startPos.x += _sortIntervel * _leftCount;
		auto count = 0;
		auto outCount = 0;
	...
	void SouthMahjongCardPool::removeCardMsg(Card* card)
	{
		...
		if (!_hasSelectOutCard && color == mahjongColor(GameManager::getInstance()->getQueMen()) || hasNoQueColor())       // 是否有缺门
		{
			...
			log("real will removeCard = %d", _willRemoveCard->getCardSumNumber());
		}
	}
	...
	void SouthMahjongCardPool::removeCardMsgCallBack()
	{
	...
	Card* SouthMahjongCardPool::getAutoSendCard()
	{
		...
		auto cardIter = std::find_if(_listCard.begin(), _listCard.end(), [&](Card * card){
			return card->getCardColor() == mahjongColor(GameManager::getInstance()->getQueMen());});
	...
	bool SouthMahjongCardPool::canHu(HUPAI_TYPE_EX& typeBase, HUPAI_TYPE_EX& typeAdd, bool& hasAdd, Card* card)
	{
		...
		// base
		if (isLongQingQiDui(card))   // 清带幺
		{
			typeBase = HUPAI_TYPE_EX::HUPAI_QING_LONG_QI_DUI;
			return true;
		}
		if (isQingYaoJiu(card))   // 清带幺
		{
			typeBase = HUPAI_TYPE_EX::HUPAI_QING_DAI_YAO;
			return true;
		}
		if (isQingQiDui(card))    // 清七对
		{
			typeBase = HUPAI_TYPE_EX::HUPAI_QING_QI_DUI;
			return true;
		}
		if (isLongQiDui(card))   // 龙七对
		{
			typeBase = HUPAI_TYPE_EX::HUPAI_LONG_QI_DUI;
			return true;
		}
		if (isQingDui(card))         // 清对
		{
			typeBase = HUPAI_TYPE_EX::HUPAI_QING_DA_DUI;
			return true;
		}
		if (isJiangDui(card))         // 将对
		{
			typeBase = HUPAI_TYPE_EX::HUPAI_JIANG_DA_DUI;
			return true;
		}
		if (isDaiJiuYao(card))         // 带幺九
		{
			typeBase = HUPAI_TYPE_EX::HUPAI_QUAN_DAI_YAO;
			return true;
		}
		if (isXiaoQiDui(card))       // 小七对
		{
			typeBase = HUPAI_TYPE_EX::HUPAI_AN_QI_DUI;
			return true;
		}
		if (isQingYiSe(card))         // 清一色
		{
			typeBase = HUPAI_TYPE_EX::HUPAI_QING_YI_SE;
			return true;
		}
		if (isDuiDuiHu(card))          // 对对胡
		{
			typeBase = HUPAI_TYPE_EX::HUPAI_PENG_PENG_HU;
			return true;
		}
		if (isPingHu(card))             // 平胡
		{
