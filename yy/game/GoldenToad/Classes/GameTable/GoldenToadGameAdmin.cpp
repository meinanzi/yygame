...
{
	static const char* ADMINUI_PATH			= "goldenToad/AdminUi/AdminNode.csb";
	static const char* LIST_BG1				= "goldenToad/AdminUi/Res/listItem_1.png";               //listBG
	static const char* LIST_BG2				= "goldenToad/AdminUi/Res/listItem_2.png";               //listBG
	static const char* LIST_BG3				= "goldenToad/AdminUi/Res/listItem_3.png";               //listBG
	...
	//创建受控玩家列表
	void GameAdmin::createControlList(std::vector<ControlInfo> controls)
	{
		_ListView_Control->removeAllItems();

		for (auto info : controls)
		{
			if (0 != info.user_id_)
			{
				addControlItem(&info);
	...
	//创建特殊鱼受控玩家列表
	void GameAdmin::createSprcialList(std::vector<SpecialUser> specias)
	{
		_ListView_UserID->removeAllItems();
		_userList.clear();

		int n = 0;
		for (auto info : specias)
		{
			if (0 != info.user_id_)
			{
				addSpecialItem(info, n++);

				auto TextField_UserID = dynamic_cast<TextField*>(_adminUI.Image_SpecialBg->getChildByName("TextField_UserID"));
				if (info.user_id_ == atoi(TextField_UserID->getString().c_str()))
				{
					_ListView_Fish->removeAllItems();

					for (int i = 0; i < 4; i++)
					{
						auto fish = info.special_fish_[i];
	...
	void GameAdmin::addOnlineItem(OnLineUserInfo info)
	{
		...
		//玩家游戏币
		sprintf(str, "%lld", info.i64UserMoney);
