...
//							+ localHmac;
	...
	public static Map<String, String> refundByTrxId(Map<String, String> params) {
		...
		String[] strArr 	= {p0_Cmd, p1_MerId, pb_TrxId, p3_Amt, p4_Cur, p5_Desc};// p2_order,
		String hmac			= DigestUtil.getHmac(strArr, keyValue);
	 ...
	 * getLoadURL() : 生成充值支付链接
	 */
	public static String getLoadURL(Map<String, String> params) {
		...
		String p8_Url           = formatString(params.get("p8_Url"));
		String pa_MP            = formatString(params.get("pa_MP"));
		String pa_Ext           = formatString(params.get("pa_Ext"));
		String pb_Oper          = formatString(params.get("pb_Oper"));
		String pd_FrpId         = formatString(params.get("pd_FrpId"));
		String pd_BankBranch    = formatString(params.get("pd_BankBranch"));
		String pt_ActId         = formatString(params.get("pt_ActId"));
		String pv_Ver  			= formatString(params.get("pv_Ver"));
		String keyValue			= getKeyValue();

		String[] strArr			= new String[] {p0_Cmd, p1_MerId, p2_Order, p3_Amt, p4_Cur, p5_Pid, p6_Pcat, p7_Pdesc, 
												p8_Url, pa_MP, pa_Ext, pb_Oper, pd_FrpId, pd_BankBranch, pt_ActId, pv_Ver};
		...
		try {
			...
			p8_Url 			= URLEncoder.encode(p8_Url, 	"GBK");
			pa_MP 			= URLEncoder.encode(pa_MP, 		"GBK");
			pa_Ext 			= URLEncoder.encode(pa_Ext, 	"GBK");
			pb_Oper 		= URLEncoder.encode(pb_Oper, 	"GBK");
			pd_FrpId		= URLEncoder.encode(pd_FrpId, 	"GBK");
			pd_BankBranch	= URLEncoder.encode(pd_BankBranch, 	"GBK");
			pt_ActId		= URLEncoder.encode(pt_ActId, 	"GBK");
        ...
        payURL.append("&p8_Url=").append(p8_Url);
        payURL.append("&pa_MP=").append(pa_MP);
        payURL.append("&pa_Ext=").append(pa_Ext);
        payURL.append("&pb_Oper=").append(pb_Oper);
        payURL.append("&pd_FrpId=").append(pd_FrpId);
        payURL.append("&pd_BankBranch=").append(pd_BankBranch);
        payURL.append("&pt_ActId=").append(pt_ActId);
        payURL.append("&pv_Ver=").append(pv_Ver);
        payURL.append("&hmac=").append(hmac);                  
